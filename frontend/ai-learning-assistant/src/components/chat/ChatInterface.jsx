...
const ChatInterface = () => {
  ...
  const renderMessage = (msg, index) => {
    ...
    return (
      <div key={index}  className={`flex items-start gap-3 my-4 ${isUser ? "justify-end" : ''}`}>
        {!isUser && (
          <div className="w-9 h-9 rounded-xl bg-linear-to-br from-emerald-400 to-teal-500 shadow-lg shadow-emerald-500/25
        ...
        {isUser && (
          ...
          text-slate-700 font-semibold text-sm shrink-0 shadow-sm">
  ...
  if (initialLoading) {
    return (
      ...
      items-center justify-center shadow-xl shadow-slate-200/50">
  ...
  return (
    ...
    shadow-slate-200/50 overflow-hidden">
      ...
      <div className="flex-1 p-6 overflow-y-auto bg-linear-to-br from-slate-50/50 via-white/50 to-slate-50/50">
        {history.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className="w-16 h-16 rounded-2xl bg-linear-to-br from-emerald-100 to-teal-100 flex items-center
            justify-center mb-4 shadow-lg shadow-emerald-500/10">
      ...
      {/* Input Area */}
      <div className="p-5 border-t border-slate-200/50 bg-white/80">
        <form onSubmit={handleSendMessage} className='flex items-center gap-3'>
          <input 
            ...
            className='flex-1 h-12 px-4 border-2 border-slate-200 rounded-xl bg-slate-50/50 text-slate-900
          ...
          <button
            ...
            className='shrink-0 w-12 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600
            hover:to-teal-600 text-white rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25
